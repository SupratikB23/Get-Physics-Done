...
- **Observability streams** (`GPD/observability/`): Historical event logs are primarily machine-local telemetry. They travel with git, but `GPD/observability/current-execution.json` contributes to the portable bounded-segment resume state only when its `resume_file` still resolves to a repo-local path on the current machine. Otherwise it is only a portable bounded-segment resume hint. Non-project or missing resume pointers are treated as advisory telemetry rather than authoritative resume state.
