...
`must_surface` is a boolean scalar. Use the YAML literals `true` and `false`; do not quote them or replace them with synonyms. If `must_surface: true`, `required_actions[]` and `applies_to[]` must both stay non-empty. `carry_forward_to[]` is free-text workflow scope only and must not be overloaded with contract IDs. `uncertainty_markers` must stay a YAML object, not a string or list.
