...
def test_active_runtime_command_prefix_falls_back_to_descriptor_command_prefix(monkeypatch) -> None:
    descriptor = SimpleNamespace(
        public_command_surface_prefix="",
        command_prefix="/fallback:",
    )
    monkeypatch.setattr(public_surface_contract_module, "local_cli_install_local_example_command", lambda: "install stub")
    ...
    assert cli_module._active_runtime_command_prefix(cwd=Path("/tmp")) == "/fallback:"
